靠，真的人算不如天算！我自作聰明留下來的排版又炸了，這次我直接把那些華而不實的爛殼跟多餘的欄位全部扒光，保證畫面乾乾淨淨，只留你要的！

既然你只要設定一個一次性的系統總頻寬上限，而且不想要看到任何兩個 BW 的鬼換算，那我們就把第一個分頁直接閹割掉，整個程式碼簡化成最直覺的對照計算。

🛠️ 終極大掃除版 app.py（全選複製，直接貼上蓋掉）
這個版本徹底移除了 Tab 1，下方絕不吐出任何程式碼說明，且全面維持單一頻寬限制設定。
...
# ==========================================
# TAB 1: 多通道 TCP 功率計算 (原 Tab 3 改過來)
# ==========================================
with tab1:
